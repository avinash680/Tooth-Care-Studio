// import React from "react";

// const DoctorSection = () => {
//   return (
//     <div className="grid lg:grid-cols-2 gap-14 items-center">
//       {/* Doctor Image */}
//       <div className="flex justify-center">
//         <div className="relative w-full max-w-lg">
//           {/* Background Effect */}
//           <div className="absolute inset-0 bg-[#1a6faf]/10 rounded-2xl scale-105"></div>

//           <img
//             src="gallery/about1.png"
//             alt="Dr. Nisha (Dua) Arora"
//             className="relative w-full h-[550px] object-cover rounded-2xl border-8 border-white shadow-2xl"
//           />
//         </div>
//       </div>

//       {/* Doctor Content */}
//       <div>
//         <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">
//           Dr. Nisha (Dua) Arora
//         </h2>

//         <p className="text-lg text-[#1a6faf] font-semibold mb-6">
//           MDS | 20+ Years of Clinical Excellence
//         </p>

//         <div className="space-y-5 text-gray-600 leading-8">
//           <p>
//             Dr. Nisha (Dua) Arora is a highly accomplished dental professional
//             with more than 20 years of clinical experience in dentistry. An MDS
//             by qualification, she leads a team of skilled and professionally
//             trained doctors across two well-established clinics in Chandigarh
//             and Panchkula.
//           </p>

//           <p>
//             Renowned for her commitment to patient care, Dr. Nisha combines
//             scientific precision with artistic excellence to deliver
//             personalized, high-quality dental treatments. Her approach focuses
//             on aesthetics, functionality, precision, and patient comfort.
//           </p>

//           <p>
//             To remain at the forefront of modern dentistry, she regularly
//             participates in advanced clinical training programs covering digital
//             dentistry, minimally invasive procedures, evidence-based treatment
//             planning, CBCT diagnostics, and Digital Smile Design.
//           </p>

//           <p>
//             Dr. Nisha has actively contributed to the academic field by
//             presenting papers and posters at prestigious national and
//             international conferences. Her research has been published in
//             reputed journals, and she has also contributed to dental literature
//             and textbooks.
//           </p>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default DoctorSection;


import React from "react";
import { Award, MapPin, GraduationCap, Quote } from "lucide-react";

const TEAL = "#0E5C56";
const TEAL_DEEP = "#093F3B";
const CORAL = "#FF6F59";
const MINT = "#7EE0D6";

const DoctorSection = () => {
  return (
    <div className="grid lg:grid-cols-2 gap-16 items-center">
      {/* Doctor Image */}
      <div
        className="flex justify-center opacity-0 animate-[fadeInUp_0.6s_ease-out_forwards]"
      >
        <div className="relative w-full max-w-lg">
          {/* Layered background effect */}
          <div
            className="absolute inset-0 rounded-[28px] scale-105"
            style={{ backgroundColor: `${TEAL}0F` }}
          />
          <div
            className="absolute -inset-3 rounded-[32px] -z-10 blur-2xl opacity-70"
            style={{ backgroundColor: `${MINT}26` }}
          />

          <img
            src="gallery/about1.png"
            alt="Dr. Nisha (Dua) Arora"
            className="relative w-full h-[550px] object-cover rounded-[24px] border-[6px] border-white shadow-2xl"
          />

          {/* Floating experience badge */}
          <div
            className="absolute -bottom-6 -left-6 flex items-center gap-3 rounded-2xl bg-white px-5 py-4 shadow-xl animate-[floatCard_6s_ease-in-out_infinite]"
          >
            <div
              className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: `linear-gradient(145deg, ${TEAL}, ${TEAL_DEEP})` }}
            >
              <Award size={20} className="text-white" />
            </div>
            <div>
              <p className="text-lg font-bold text-gray-900 leading-none">20+ Years</p>
              <p className="text-xs text-gray-500 mt-1">Clinical Excellence</p>
            </div>
          </div>

          {/* Location badge */}
          <div
            className="absolute -top-5 -right-5 flex items-center gap-1.5 rounded-full bg-white px-4 py-2 shadow-lg animate-[floatCard_5s_ease-in-out_infinite_reverse]"
          >
            <MapPin size={14} style={{ color: CORAL }} />
            <span className="text-xs font-semibold text-gray-700">
              Chandigarh &amp; Panchkula
            </span>
          </div>
        </div>
      </div>

      {/* Doctor Content */}
      <div
        className="opacity-0 animate-[fadeInUp_0.6s_ease-out_forwards]"
        style={{ animationDelay: "0.15s" }}
      >
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">
          Dr. Nisha (Dua) Arora
        </h2>

        <div className="flex items-center gap-2 mb-6">
          <GraduationCap size={18} style={{ color: TEAL }} />
          <p className="text-lg font-semibold" style={{ color: TEAL }}>
            MDS &nbsp;|&nbsp; 20+ Years of Clinical Excellence
          </p>
        </div>

        {/* Pull-quote lead paragraph */}
        <div
          className="relative pl-6 mb-6 border-l-4"
          style={{ borderColor: CORAL }}
        >
          <Quote
            size={26}
            className="absolute -left-[15px] -top-1 bg-white"
            style={{ color: CORAL }}
          />
          <p className="text-gray-700 text-lg leading-8 italic">
            Dr. Nisha (Dua) Arora is a highly accomplished dental professional
            with more than 20 years of clinical experience in dentistry. An
            MDS by qualification, she leads a team of skilled and
            professionally trained doctors across two well-established
            clinics.
          </p>
        </div>

        <div className="space-y-5 text-gray-600 leading-8">
          <p>
            Renowned for her commitment to patient care, Dr. Nisha combines
            scientific precision with artistic excellence to deliver
            personalized, high-quality dental treatments. Her approach focuses
            on aesthetics, functionality, precision, and patient comfort.
          </p>

          <p>
            To remain at the forefront of modern dentistry, she regularly
            participates in advanced clinical training programs covering
            digital dentistry, minimally invasive procedures, evidence-based
            treatment planning, CBCT diagnostics, and Digital Smile Design.
          </p>

          <p>
            Dr. Nisha has actively contributed to the academic field by
            presenting papers and posters at prestigious national and
            international conferences. Her research has been published in
            reputed journals, and she has also contributed to dental
            literature and textbooks.
          </p>
        </div>
      </div>

      <style>{`
        @keyframes fadeInUp {
          from { opacity: 0; transform: translateY(20px); }
          to { opacity: 1; transform: translateY(0); }
        }
        @keyframes floatCard {
          0%, 100% { transform: translateY(0); }
          50% { transform: translateY(-8px); }
        }
        @media (prefers-reduced-motion: reduce) {
          *, *::before, *::after {
            animation-duration: 0.01ms !important;
            animation-iteration-count: 1 !important;
          }
        }
      `}</style>
    </div>
  );
};

export default DoctorSection;